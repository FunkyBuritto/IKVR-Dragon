    ...
    public class GaiaResource
    {
            ...
            switch (resourceType)
            {
                case GaiaConstants.SpawnerResourceType.TerrainTexture:
                    ...
                    foreach (TerrainLayer proto in terrain.terrainData.terrainLayers)
                    {
                        if (splat != null && proto != null)
                        {
                            ...
                            {
                                if (PWCommon4.Utils.IsSameTexture(splat.m_texture, proto.diffuseTexture, false) == true)
                                {
                ...
                case GaiaConstants.SpawnerResourceType.TerrainDetail:
                    ...
                    foreach (DetailPrototype proto in terrain.terrainData.detailPrototypes)
                    {
                        ...
                        {
                            if (PWCommon4.Utils.IsSameTexture(detail.m_detailTexture, proto.prototypeTexture, false))
                            {
                                return localTerrainIdx;
                            }
                            if (PWCommon4.Utils.IsSameGameObject(detail.m_detailProtoype, proto.prototype, false))
                            {
                                return localTerrainIdx;
                            }
                        }
                ...
                case GaiaConstants.SpawnerResourceType.TerrainTree:
                    ...
                    {
                        if (PWCommon4.Utils.IsSameGameObject(tree.m_desktopPrefab, proto.prefab, false))
                        {
                            return localTerrainIdx;
                        }
                        localTerrainIdx++;
        ...
        public void AddMissingPrototypesToTerrain(Terrain terrain)
        {
            ...
            foreach (ResourceProtoTexture splat in m_texturePrototypes)
            {
                ...
                {
                    if (PWCommon4.Utils.IsSameTexture(sp.texture, splat.m_texture, false))
                    {
                        found = true;
                    }
                }
            ...
            foreach (ResourceProtoDetail detail in m_detailPrototypes)
            {
                ...
                foreach (DetailPrototype dp in terrainDetails)
                {
                    ...
                    {
                        if (PWCommon4.Utils.IsSameTexture(dp.prototypeTexture, detail.m_detailTexture, false))
                        {
                            found = true;
                        }
                        if (PWCommon4.Utils.IsSameGameObject(dp.prototype, detail.m_detailProtoype, false))
                        {
                            found = true;
                        }
                    }
            ...
            foreach (ResourceProtoTree tree in m_treePrototypes)
            {
                ...
                {
                    if (PWCommon4.Utils.IsSameGameObject(tp.prefab, tree.m_desktopPrefab, false))
                    {
                        found = true;
                    }
                }
